...
if (!function_exists('mo_entry_comments_link')) {
    function mo_entry_comments_link($args = array()) {
        ...
        $defaults = array('zero' => esc_html__('No Comments', 'peak'), 'one' => esc_html__('%1$s Comment', 'peak'), 'more' => esc_html__('%1$s Comments', 'peak'), 'css_class' => 'comments-link', 'none' => esc_html__('Comments Disabled', 'peak'), 'before' => '', 'after' => '');
        ...
        elseif (0 == $num_of_comments)
            $comments_link .= '<a href="' . get_permalink() . '#respond" title="' . sprintf(esc_html__('Comment on %1$s', 'peak'), the_title_attribute('echo=0')) . '">' . sprintf($args['zero'], number_format_i18n($num_of_comments)) . '</a>';
        elseif (1 == $num_of_comments)
            $comments_link .= '<a href="' . get_comments_link() . '" title="' . sprintf(esc_html__('Comment on %1$s', 'peak'), the_title_attribute('echo=0')) . '">' . sprintf($args['one'], number_format_i18n($num_of_comments)) . '</a>';
        elseif (1 < $num_of_comments)
            $comments_link .= '<a href="' . get_comments_link() . '" title="' . sprintf(esc_html__('Comment on %1$s', 'peak'), the_title_attribute('echo=0')) . '">' . sprintf($args['more'], number_format_i18n($num_of_comments)) . '</a>';
...
if (!function_exists('mo_entry_comments_number')) {
    ...
    function mo_entry_comments_number($args = array()) {
        ...
        $defaults = array('zero' => esc_html__('No Comments', 'peak'), 'one' => esc_html__('%1$s Comment', 'peak'), 'more' => esc_html__('%1$s Comments', 'peak'), 'css_class' => 'comments-number', 'none' => '', 'before' => '', 'after' => '');
...
if (!function_exists('mo_comment_author')) {
    function mo_comment_author() {
        ...
        if ($url)
            $output = '<cite class="fn" title="' . esc_attr($author) . '"><a href="' . esc_url($url) . '" title="' . esc_attr($author) . '" class="url" rel="external nofollow">' . esc_html($author) . '</a></cite>';
        else
            $output = '<cite class="fn">' . esc_html($author) . '</cite>';
...
if (!function_exists('mo_comment_published')) {
    ...
    function mo_comment_published() {
        $link = '<span class="published">' . sprintf(esc_html__('%1$s at %2$s', 'peak'), '<abbr class="comment-date" title="' . get_comment_date(esc_html__('l, F jS, Y, g:i a', 'peak')) . '">' . get_comment_date() . '</abbr>', '<abbr class="comment-time" title="' . get_comment_date(esc_html__('l, F jS, Y, g:i a', 'peak')) . '">' . get_comment_time() . '</abbr>') . '</span>';
        return $link;
    }
}
...
if (!function_exists('mo_comment_reply_link')) {
    ...
    function mo_comment_reply_link() {
        ...
        $args = array(
            'reply_text' => esc_html__('Reply', 'peak'),
            'login_text' => esc_html__('Log in to reply.', 'peak'),
...
if (!function_exists('mo_comment_edit_link')) {
    ...
    function mo_comment_edit_link() {
        ...
        $link = '<a class="comment-edit-link" href="' . esc_url($edit_link) . '" title="' . sprintf(esc_html__('Edit %1$s', 'peak'), $comment->comment_type) . '"><span class="edit">' . esc_html__('Edit', 'peak') . '</span></a>';
                ...
                <div class="comment-text-wrap">
                    ...
                    <div class="entry-content comment-text">
                        ...
                        <?php if ($comment->comment_approved == '0') : ?>
                            <?php echo '<p class="alert moderation">' . esc_html__('Your comment is awaiting moderation.', 'peak') . '</p>'; ?>
                        <?php endif; ?>
