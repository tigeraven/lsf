...
class nxs_snapClassNT {    
    ...
    public function showNTLine($indx, $pbo) { if (!isset($pbo['aName'])) $pbo['aName'] = ''; if (!isset($pbo['do']) && isset($pbo['do'.$this->ntInfo['code']])) $pbo['do'] = $pbo['do'.$this->ntInfo['code']]; 
      ...
      <div id="dom<?php echo $this->ntInfo['code'].$indx; ?>Div">
        <div style="margin:0px;margin-left:5px;"> <img id="<?php echo $this->ntInfo['code'].$indx;?>LoadingImg" style="display: none;" src='<?php echo NXS_PLURL; ?>img/ajax-loader-sm.gif' />
          <?php if ((int)$pbo['do'] == 1 && ((isset($pbo['fltrsOn']) && (int)$pbo['fltrsOn'] == 1))) { ?> 
            <input type="radio" id="rbtn<?php echo $this->ntInfo['lcode'].$indx; ?>" value="1" name="<?php echo $this->ntInfo['lcode']; ?>[<?php echo $indx; ?>][do]" checked="checked" class="nxs_acctcb" onmouseout="nxs_hidePopUpInfo('popOnlyCat');" onmouseover="nxs_showPopUpInfo('popOnlyCat', event);" /> 
          <?php } else { ?>            
            <input value="0" name="<?php echo $this->ntInfo['lcode']; ?>[<?php echo $indx; ?>][do]" type="hidden" />             
            <input value="1" name="<?php echo $this->ntInfo['lcode']; ?>[<?php echo $indx; ?>][do]" type="checkbox" class="nxs_acctcb" <?php if ((int)$pbo['do'] == 1) echo "checked"; ?> />             
          <?php } ?>              
    ...
    public function elemEdTitleFormat($ii,$l,$msgTFormat) { $nt = $this->ntInfo['lcode']; ?>
      <tr class="<?php echo 'nxstbldo'.strtoupper($nt).$ii; ?>" ><th scope="row" class="nxsTHRow"><?php echo $l; ?></th><td>                
          <input name="<?php echo $nt; ?>[<?php echo $ii; ?>][<?php echo $fn; ?>]" id="<?php echo $nt.$ii; ?>SNAPformatT" style="width: 50%;" value="<?php echo htmlentities($msgTFormat, ENT_COMPAT, "UTF-8"); ?>" onfocus="nxs_showFrmtInfo('msgFrmtT<?php echo $nt.$ii; ?>');" /><?php nxs_doShowHint("msgFrmtT".$nt.$ii); ?></td>
        </tr><?php                        
    }
    ...
    function nxs_tmpltAddPostMeta($post, $ntOpt, $pMeta){ $nt = $this->ntInfo['lcode']; $ntU = $this->ntInfo['code']; $ntName = $this->ntInfo['name']; $ii = $ntOpt['ii']; $doNT = $ntOpt['do'];  ?>
      <tr <?php if ($ntOpt['fltrsOn']=='1') { ?> onmouseout="hidePopShAtt('FLT');" onmouseover="showPopShAtt('FLT', event);" <?php } ?>><th style="text-align:left;" colspan="2"> <?php 
      if ($post->post_status != "publish" && $ntOpt['fltrsOn']=='1'){ ?>
        <input type="radio" id="rbtn<?php echo $ntU.$ii; ?>" value="1" name="<?php echo $nt; ?>[<?php echo $ii; ?>][do]" checked="checked" class="nxsGrpDoChb" /> <?php } 
      else { ?>
    ...
    public function adjMetaOptG($optMt, $pMeta) { $optMt['isPosted'] = isset($pMeta['isPosted'])?$pMeta['isPosted']:''; if (isset($pMeta['postType'])) $optMt['postType'] = $pMeta['postType']; 
      ...
      if (isset($pMeta['do'])) $optMt['do'] = $pMeta['do'] == 1?1:0; else { if (isset($pMeta['msgFormat'])) $optMt['do'] = 0; } // What is that?
