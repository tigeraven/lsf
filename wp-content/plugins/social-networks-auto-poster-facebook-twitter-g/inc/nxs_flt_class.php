...
class nxs_Filters {
    ...
    public static function init($od=false) { 
        $screen = get_current_screen(); // prr($screen); var_dump($od);
        
        if($od || ( !empty($screen) && ( $screen->id == 'settings_page_NextScripts_SNAP' || $screen->id == 'snapautoposter_page_nxssnap-settings' || $screen->id == 'snap_page_nxssnap-reposter' || $screen->id == 'snapautoposter_page_nxssnap-reposter' || $screen->id == 'snapautoposter_page_nxssnap-accounts'))) { 
    ...
    public static function enqueue() {
        ...
        if( $screen->id == 'nxs_filter' || $screen->id == 'snapautoposter_page_nxssnap-accounts' || $screen->id == 'snapautoposter_page_nxssnap-settings' || $screen->id == 'snap_page_nxssnap-reposter' || $screen->id == 'snapautoposter_page_nxssnap-reposter') { 
   ...
   <div class="nxs_tls_bdX">          
     ...
     <div id="riS<?php echo $nt; ?><?php echo $ii; ?>xd"  style="padding-top: 5px; padding-left: 0px;<? if (isset($options['rpstType']) && $options['rpstType']=='1') echo "display:none;"; ?>"><b><?php _e('When finished', 'social-networks-auto-poster-facebook-twitter-g'); ?>:</b>       
    ...
    //## Sections
    public static function print_catsTags_section( $current_post, $metaSettings, $nt='', $ii='' ) {  // ## Categories and Tags
        ...
        $tags = get_tags( array( 'hide_empty' => false, 'include'=>implode(',',$metaSettings['nxs_tags_names']), 'number'=>1000 ) );
        $categories = get_categories( array( 'hide_empty' => false, 'include'=>implode(',',$metaSettings['nxs_tags_names']), 'number'=>1000 ) ); 
        ...
        echo '&nbsp;<input type="radio" '.$selCI.' name="'.self::makeInputName('_ie_cats_names', $nt, $ii).'" value="0">Include&nbsp;&nbsp;<input type="radio" '.$selCE.' name="'.self::makeInputName('_ie_cats_names', $nt, $ii).'" value="1">Exclude<br/>';
    ...
    public static function print_types_section( $current_post, $metaSettings, $nt='', $ii='' ) {   $ntN = $nt.$ii;
        ...
        */   
        if( !empty( self::$posts_types ) ) {
            echo '<div><label class="field_title">'. __( 'Type', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label>&nbsp;&nbsp;<span class="description">'. __( 'Select posts by type' ) .'</span><br/>';
        ...
        if( !empty( $formats ) ) {
            echo '<div><label class="field_title">'. __( 'Format', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label>&nbsp;&nbsp;<span class="description">'. __( 'Select posts by format' ) .'</span><br/>';
    ...
    // ## Author    
        ...
        $users = get_users(); $user_names = array();        
        if( $users ) foreach( $users as $user )  $user_names[$user->ID] = $user->user_login;        
        if( !empty( $user_names ) ) {            
    ...
    public static function print_taxonomies_section( $current_post, $metaSettings, $nt='', $ii='' ) { $isVis = !empty($metaSettings['nxs_term_names']); 
        ...
        for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i == 1 ? '' : '_'. $i;    $rel = $i == 1 ? '' : 'nxs_term_compare_'. $i; if( $i > 1 ) echo '<hr>';
          ...
          if( !empty( $terms_names ) ) {
            ...
            echo '<div class="'. 'nxs_short_field" rel="'. $rel .'">';
            echo '<label class="field_title">'. __( 'Operator', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label>&nbsp;&nbsp;<span class="description">'. __( 'Compare Operator' ) .'</span>';
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $compare_options, "nxs_term_operator$postfix", !empty($metaSettings["nxs_term_operator$postfix"])?$metaSettings["nxs_term_operator$postfix"]:'', false, false, self::makeInputName("_term_operator$postfix", $nt, $ii) ); echo '</div>';
            ...
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $children_options, "nxs_term_children$postfix", !empty($metaSettings["nxs_term_children$postfix"])?$metaSettings["nxs_term_children$postfix"]:'', true, false, self::makeInputName("_term_children$postfix", $nt, $ii) ); echo '</div>';
        ...
        self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $relation_options, "nxs_term_relation", $metaSettings, false, false, self::makeInputName("_term_relation", $nt, $ii )); echo '</div>';
...
function get_posts_ids_by_filter( $filter ) { $args = array(); //prr($filter);
    ...
    if( !empty( $filter['nxs_post_formats'] ) ) { $post_formats  = $filter['nxs_post_formats']; $formats_query = array( 'taxonomy' => 'post_format', 'terms' => $post_formats, 'field' => 'slug' );        
        ...
        if ( in_array( 'standard', $post_formats ) ) { $reg_formats = get_theme_support( 'post-formats' );            
