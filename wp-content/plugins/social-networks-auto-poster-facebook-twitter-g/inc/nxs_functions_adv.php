...
if (!function_exists("nxs_snapAjax")) { function nxs_snapAjax() { check_ajax_referer('nxsSsPageWPN'); $arg = ''; nxs_Filters::init(true);
  ...
  if ($_POST['nxsact']=='setNTset') { global $nxs_snapAvNts; unset($_POST['action']); unset($_POST['nxsact']); unset($_POST['_wp_http_referer']); unset($_POST['_wpnonce']); //unset($_POST['apDoSFB0']); // Do something
    ...
    }  nxs_save_ntwrksOpts($networks); $plgn_NS_SNAutoPoster->nxs_options = $networks; //prr($_POST['xi']);  prr($networks['xi']); // die('OK');
  } 
...
if (!function_exists("nxs_snapCheckFilters")) { function nxs_snapCheckFilters($options, $postObj) { $postID = $postObj->ID; //  prr($options);  prr($postObj);
  if (!empty($options['fltrsOn']) && !empty($options['fltrs']) && empty($options['fltrAfter'])) { $options['fltrs']['nxs_postID'] = $postID; $options['fltrs']['fullreturn']='1';// echo "|Pre FLT 2|"; 
    add_filter( 'pre_get_posts', 'nxs_noSing' ); $pfidRet = get_posts_ids_by_filter( $options['fltrs'] ); /* prr($pfidRet); */  $pfid = $pfidRet['p']; remove_filter( 'pre_get_posts', 'nxs_noSing' ); // echo "|W|";
    if (empty($pfid) || empty($pfid[0]) || $pfid[0]!=$postID) { $msg = nxsAnalyzePostFilters($postObj, $options['fltrs']); 
      $postLogRec = "ID: ".$postObj->ID." | ".$postObj->post_title." (".$postObj->post_name.") | Author(ID): ".$postObj->post_author." |Status: ".$postObj->post_status.
        " | Format ".$postObj->filter." | Post Type: ".$postObj->post_type." | ";
      return "\r\n<br/>".'| Args: '.print_r($pfidRet['a'], true).' '."\r\n<br/>".'| Query: '.$pfidRet['q']."\r\n<br/>".'| Post: '.$postLogRec;
    }
  } return false;   
}}
if (!function_exists("nxsAnalyzePostFilters")) { function nxsAnalyzePostFilters($post, $filter) { //## ????? What is this function?????
    //## Cats
...
if (!function_exists("nxs_mkRemOptsArr")) {function nxs_mkRemOptsArr($hdrsArr, $ck='', $flds='', $rdr=0, $timt=45, $sslverify = false){ 
  $a = array('headers' => $hdrsArr, 'httpversion' => '1.1', 'timeout' => $timt, 'redirection' => $rdr, 'sslverify'=>$sslverify); if (!empty($flds)) $a['body'] = $flds; if (!empty($ck)) $a['cookies'] = $ck; return $a;
}}
